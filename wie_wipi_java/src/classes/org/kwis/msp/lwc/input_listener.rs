...
impl InputListener {
    ...
    async fn init(jvm: &Jvm, _: &mut WieJvmContext, mut this: ClassInstanceRef<Self>, owner: ClassInstanceRef<TextComponent>) -> JvmResult<()> {
        ...
        jvm.put_field(&mut this, "__wieChanged", "Z", false).await?;
    ...
    async fn notify_text_changed(
        ...
        let owner: ClassInstanceRef<TextComponent> = jvm.get_field(&this, "__wieTextComponent", "Lorg/kwis/msp/lwc/TextComponent;").await?;

        if owner.is_null() {
            return Err(jvm.exception("java/lang/NullPointerException", "").await);
        }

        let cursor: i32 = jvm.get_field(&owner, "m_cPos", "I").await?;
        ...
        match change_type {
            -1 => {
                ...
                let max_length: i32 = jvm.get_field(&owner, "maxLength", "I").await?;

                if max_length > 0 {
                    let text: ClassInstanceRef<java_runtime::classes::java::lang::String> =
                        jvm.get_field(&owner, "text", "Ljava/lang/String;").await?;

                    let text_length: i32 = jvm.invoke_virtual(&text, "length", "()I", ()).await?;

                    if max_length < text_length + count {
                        jvm.put_field(&mut this, "__wieChanged", "Z", true).await?;

                        if count == 2 {
                            let _: () = jvm.invoke_virtual(&owner, "insert", "([CIII)V", (data, 0, 1, cursor)).await?;
                        }

                        let _: bool = jvm.invoke_virtual(&owner, "keyNotify", "(II)Z", (1, -99)).await?;

                        return Ok(());
                    }
                }

                jvm.invoke_virtual(&owner, "insert", "([CIII)V", (data, 0, count, cursor)).await
            }
            ...
            1 => {
                ...
                jvm.invoke_virtual(&owner, "delete", "(II)V", (cursor - length, length)).await
            }
